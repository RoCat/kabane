...
export class KabaneBoard extends MobxReactionUpdate(LitElement) {
  static styles = [
    themeStyles,
    resetStyles,
    css`
  ...
  private getBacklogTickets(): Ticket[] {
    return ticketManager.tickets.filter(
  ...
  private getColumnTickets(column: KanbanColumn): Ticket[] {
    return ticketManager.tickets.filter(ticket => {
      ...
      if (versionsManager.selectedVersionId) {
        return ticket.version === versionsManager.selectedVersionId;
      }
  ...
  private handleTicketMove(e: CustomEvent) {
    ...
    // Find the ticket
    const ticket = ticketManager.tickets.find(t => t.id === ticketId);
    if (!ticket) return;
    ...
    ticketManager.updateTicket(ticket, changes);
  }
  ...
  render() {
    ...
    const columns = appMobx.config?.columns || this.getDefaultColumns();
    const backlogTickets = this.getBacklogTickets();

    return html`
      <div class="version-bar">
        <kabane-version-selector></kabane-version-selector>

        ${!appMobx.canEdit
          ? html`
              <button class="btn-new-ticket" @click=${() => appMobx.navigateTo('/create-ticket')}>
                <span>+</span>
          ...
          ${columns.map(
            column => html`
              <kabane-column
                ...
                ?readonly=${!appMobx.canEdit}
                .targetVersion=${versionsManager.selectedVersionId}
              ></kabane-column>
            `,
          )}
        </div>
