...
import type { Ticket, TicketTypeId, GitHubUser } from '../types';
import { uploadImage, deleteImage, isValidImageExtension, getImageAsDataUrl } from '../api/github';
import { appMobx, RouteEnum, routePatterns } from '@/utils/app-mobx';
...
export class KabaneTicketForm extends MobxReactionUpdate(LitElement) {
  static styles = [
    themeStyles,
    resetStyles,
    buttonStyles,
  ...
  private get parentCandidates(): Ticket[] {
    return ticketManager.tickets.filter(t => t.id !== this.ticket?.id);
  }
  ...
  private async loadImages() {
    if (!this.ticket?.images?.length || !appMobx.repository || !appMobx.token) return;

    const [owner, repo] = appMobx.repository.full_name.split('/');
  ...
  private initFormData() {
    ...
    const updateMatch = hash.match(routePatterns[RouteEnum.UPDATE_TICKET]);
    const ticketId = updateMatch ? updateMatch[1] : null;
    this.ticket =
      appMobx.currentRoute === RouteEnum.UPDATE_TICKET && ticketId ? this.getTicketById(ticketId) || null : null;
    if (this.ticket) {
      ...
      this.formData = {
        ...
        status: 'backlog',
        priority: undefined,
        version: versionsManager.selectedVersionId || undefined,
        assignees: [],
        labels: [],
        description: '',
      };
  ...
  private handleBack() {
    // if last page was ticket details, go back there instead of board
    if (appMobx.previousRoute === RouteEnum.TICKET_DETAIL && this.ticket) {
      appMobx.navigateTo(`/ticket/${this.ticket.id}`);
    } else {
      appMobx.navigateTo('/');
    }
  }
  ...
  private async handleSave() {
    ...
    if (this.isEditing && this.ticket) {
      ...
      await ticketManager.updateTicket(this.ticket, changes);
    } else {
      const ticket: Ticket = {
        ...this.formData,
  ...
  private getStatusOptions(): string[] {
    ...
    if (appMobx.config?.columns) {
      appMobx.config.columns.forEach(col => {
  ...
  private getFilteredContributors(): GitHubUser[] {
    ...
    return appMobx.contributors.filter(contributor => {
  ...
  private handleAssigneeKeyDown(e: KeyboardEvent) {
    ...
    const canAddCustom =
      ...
      !appMobx.contributors.some(c => c.login.toLowerCase() === this.assigneeInputValue.toLowerCase()) &&
  ...
  private getContributorByLogin(login: string): GitHubUser | undefined {
    return appMobx.contributors.find(c => c.login === login);
  }
  ...
  private getTicketById(id: string): Ticket | undefined {
    return ticketManager.tickets.find(t => t.id === id);
  }
  ...
  private async handleImageFiles(files: File[]) {
    const { token, repository } = appMobx;
    // Only handle images in edit mode (ticket must exist)
    if (!this.isEditing || !this.ticket || !repository || !token) {
    ...
    for (const file of imageFiles) {
      try {
        const imageName = await uploadImage(owner, repo, this.ticket.id, file, token, repository.default_branch);
  ...
  private async handleDeleteImage(imageName: string) {
    ...
    try {
      await deleteImage(owner, repo, this.ticket.id, imageName, token, repository.default_branch);
      ...
      <div class="page-header">
        ...
        <div class="header-actions">
          ...
          <button
            ...
            ?disabled=${appMobx.loading || Object.keys(this.errors).length > 0}
          >
            ${appMobx.loading ? msg('Saving...') : this.isEditing ? msg('Save') : msg('Create')}
          </button>
        </div>
      </div>
          ...
          <div class="field-row-3">
            ...
            <div class="field">
              ...
              >
                ...
                ${(versionsManager?.versions || []).map(v => html` <option value=${v.id}>${v.name}</option> `)}
              </select>
            </div>
