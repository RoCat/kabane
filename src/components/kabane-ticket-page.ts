...
export class KabaneTicketPage extends MobxReactionUpdate(LitElement) {
  static styles = [
    themeStyles,
    resetStyles,
    buttonStyles,
  ...
  private get childrenTickets(): Ticket[] {
    return this.ticket?.id ? ticketManager.tickets.filter(t => t.parent === this.ticket?.id) : [];
  }

  connectedCallback() {
    super.connectedCallback();
    const hash = window.location.hash.slice(1);
    const updateMatch = hash.match(routePatterns[RouteEnum.TICKET_DETAIL]);
    const ticketId = updateMatch ? updateMatch[1] : null;
    this.ticket = ticketManager.tickets.find(t => t.id === ticketId) || null;
  }

  private async loadImages() {
    if (!this.ticket?.images?.length || !appMobx.repository) return;
  ...
  private handleChildTicketClick(ticket: Ticket) {
    appMobx.navigateTo(`/ticket/${ticket.id}`);
    this.ticket = ticket;
  }
  ...
  private handleEdit() {
    appMobx.navigateTo(`/update-ticket/${this.ticket!.id}`);
  }
  ...
  private getVersionName(versionId?: string): string {
    if (!versionId || !versionsManager.versions) return '-';
    const version = versionsManager.versions.find(v => v.id === versionId);
