...
export class KabaneVersionSelector extends MobxReactionUpdate(LitElement) {
  static styles = [
    themeStyles,
    resetStyles,
    css`
  ...
  private get selectedVersion(): Version | null {
    if (!versionsManager.selectedVersionId) return null;
    return versionsManager.versions.find(v => v.id === versionsManager.selectedVersionId) || null;
  }

  private handleVersionChange(e: Event) {
    const select = e.target as HTMLSelectElement;
    const value = select.value || null;
    versionsManager.setSelectedVersionId(value);
  }
    ...
    return html`
      ...
      <div class="version-select">
        <select @change=${this.handleVersionChange}>
          ...
          ${versionsManager.versions.map(
            v => html`
              <option value=${v.id} ?selected=${v.id === versionsManager.selectedVersionId}>${v.name}</option>
            `,
          )}
        </select>
      </div>
